...
public class ESUtils {
     ...
     * @param <T>
     */
    public <T> PageInfo<T> searchPage(String index, SearchSourceBuilder searchSourceBuilder, Class<T> resultClass) {
        SearchRequest request = new SearchRequest(index);
        request.source(searchSourceBuilder);
        try {
            PageInfo<T> pageInfo = new PageInfo<>();
            pageInfo.setPageNum(searchSourceBuilder.from());
            pageInfo.setPageSize(searchSourceBuilder.size());
            SearchResponse response = restClient.search(request, RequestOptions.DEFAULT);
            SearchHits hits1 = response.getHits();
            SearchHit[] hits2 = hits1.getHits();
            List<T> retList = new ArrayList<>(hits2.length);
            ...
            pageInfo.setPages((int)Math.ceil((double) hits1.getTotalHits().value / pageInfo.getPageSize()));
            return pageInfo;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
